...
from wenyi_core.book_inspector import inspect_book
    ...
    def test_extracts_fb2_metadata_and_cover(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            Path(path).write_text(
                f"""<FictionBook xmlns:xlink="http://www.w3.org/1999/xlink">
    ...
    def test_extracts_docx_metadata(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            with zipfile.ZipFile(path, "w") as zf:
                zf.writestr(
                    "docProps/core.xml",
                    """<cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/"><dc:title>Doc Title</dc:title><dc:creator>Bob</dc:creator></cp:coreProperties>""",
                )
                zf.writestr(
                    "word/document.xml",
                    """<w:document xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main"><w:body><w:p><w:pPr><w:pStyle w:val="Heading1"/></w:pPr><w:r><w:t>Heading</w:t></w:r></w:p></w:body></w:document>""",
                )
