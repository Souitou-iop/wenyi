...
from wenyi_core.assemble.pdf_writer import _assemble_pdf_babeldoc
from wenyi_core.ingest.pdf_babeldoc import _is_image_only_pdf, read_pdf_babeldoc
from wenyi_core.pdf_bridge.client import BabeldocBridgeClient, BabeldocBridgeError
...
class BabeldocBridgeClientTests(unittest.TestCase):
    def test_fillback_uses_configured_timeout(self):
        with (
            patch(
                "wenyi_core.ingest.pdf_babeldoc.translations_from_store",
                return_value=("session", "http://bridge.test", {"0:0": "译文"}),
            ),
            patch("wenyi_core.pdf_bridge.BabeldocBridgeClient") as client_class,
        ):
    ...
    def test_health_wraps_connection_errors(self):
        ...
        with patch("wenyi_core.pdf_bridge.client.httpx.get", side_effect=OSError("down")):
    ...
    def test_extract_posts_multipart(self):
        ...
        try:
            ...
            with (
                patch("wenyi_core.pdf_bridge.client.httpx.get", return_value=health),
                patch("wenyi_core.pdf_bridge.client.httpx.post", return_value=extract) as post,
            ):
    ...
    def test_read_pdf_babeldoc_builds_segments(self):
        ...
        with (
            patch("wenyi_core.ingest.pdf_babeldoc.BabeldocBridgeClient", return_value=fake),
            patch("wenyi_core.ingest.pdf_babeldoc.toc_chapter_starts", return_value=[]),
    ...
    def test_image_only_pdf_is_rejected_before_bridge_request(self):
        with (
            patch("wenyi_core.ingest.pdf_babeldoc._is_image_only_pdf", return_value=True),
            patch("wenyi_core.ingest.pdf_babeldoc.BabeldocBridgeClient") as client,
    ...
    def test_toc_splits_chapters_by_page(self):
        from wenyi_core.ingest.pdf_babeldoc import _chapters_from_paragraphs
    ...
    def test_toc_chapter_starts_reads_weaver(self):
        from wenyi_core.ingest.pdf_babeldoc import toc_chapter_starts
