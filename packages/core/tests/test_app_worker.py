...
from wenyi_core.app_worker import EventWriter, WorkerRequest, run_worker
...
class TestRunWorker(unittest.TestCase):
    def test_completed_event_contains_outputs_and_isolated_state_directory(self):
        with tempfile.TemporaryDirectory() as root:
            ...
            with (
                patch("wenyi_core.app_worker.Config.load") as load,
                patch("wenyi_core.app_worker.Orchestrator", FakeOrchestrator),
            ):
    ...
    def test_srt_worker_delegates_to_translate_srt(self):
        with tempfile.TemporaryDirectory() as root:
            ...
            with (
                patch("wenyi_core.app_worker.Config.load") as load,
                patch("wenyi_core.srt.translate.translate_srt", fake_translate_srt),
            ):
                cfg = load.return_value
                cfg.state_dir = "old"
