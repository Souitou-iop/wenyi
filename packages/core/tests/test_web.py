...
from wenyi_core.config import Config, TierConfig
...
class TestWebAPI(unittest.TestCase):
    ...
    def test_health_and_settings_hide_saved_api_key(self):
        ...
        self.assertEqual(
            self.client.put("/api/settings", json=settings.model_dump()).status_code, 200
        )
    ...
    @patch("wenyi_core.web.OpenAI")
    def test_connection_lists_and_validates_unique_models_without_saving_draft(self, openai):
        openai.return_value.models.list.return_value = SimpleNamespace(
            data=[
    ...
    @patch("wenyi_core.web.MAX_UPLOAD_BYTES", 4)
    ...
    @patch("wenyi_core.web.inspect_book", side_effect=ValueError("broken"))
    ...
    @patch("wenyi_core.web.load_document")
    def test_upload_rejects_book_without_translatable_text(self, load_document):
        load_document.return_value = SimpleNamespace(
            chapters=[
    ...
    def test_restart_marks_orphan_export_failed_and_allows_delete(self):
        ...
        self.client.app.state.manager.save(task)
        partial_dir = self.root / "outputs" / "task-id" / "exports" / "interrupted-export"
        ...
        with TestClient(create_app(self.root)) as restarted:
            ...
            records = {record["id"]: record for record in recovered["exports"]}
    ...
    def test_chapter_review_editing_marks_outputs_stale(self):
        ...
        self.assertTrue(
            self.client.post("/api/tasks/task-id/chapters/0/review-complete").json()[
                "review_complete"
            ]
        )
    ...
    def test_glossary_crud_and_conflict_resolution(self):
        ...
        glossary.close()
        conflict = self.client.get("/api/tasks/task-id/glossary/conflicts").json()["conflicts"][0]
        ...
        self.assertEqual(
            self.client.delete("/api/tasks/task-id/glossary/terms/Rabbit").status_code,
            200,
        )
    ...
    def test_reexport_and_historical_output_download(self):
        ...
        self.assertEqual(
            self.client.get("/api/tasks/task-id/exports/historical-0/download").text,
            "old output",
        )
        ...
        self.assertFalse(self.client.get("/api/tasks/task-id/exports").json()["outputs_stale"])

    def test_reexport_failure_is_recorded_and_partial_output_is_removed(self):
        self._make_workspace()

        with patch(
            "wenyi_core.web.assemble",
        ...
        record = self.client.get("/api/tasks/task-id/exports").json()["exports"][0]
        self.assertEqual(record["status"], "failed")
        self.assertEqual(record["error"], "writer failed")
        self.assertFalse((self.root / "outputs" / "task-id" / "exports" / record["id"]).exists())
    ...
    def test_segment_edit_during_export_keeps_outputs_stale(self):
        ...
        with patch("wenyi_core.web.assemble", side_effect=blocking_assemble):
            edited, exported = asyncio.run(scenario())

        self.assertEqual(edited.status_code, 200)
        self.assertEqual(exported.status_code, 200)
        self.assertEqual(workspace.load_chapter(0).segments[0].target, "并发编辑")
        self.assertTrue(self.client.get("/api/tasks/task-id/exports").json()["outputs_stale"])
    ...
    def test_parallel_exports_preserve_both_records(self):
        ...
        exports = self.client.get("/api/tasks/task-id/exports").json()["exports"]
...
class TestTaskManager(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_worker_config_uses_selected_provider(self):
        ...
        with patch(
            "wenyi_core.web.asyncio.create_subprocess_exec",
        ...
        self.assertTrue(
            getattr(config.pipeline, "autofix_severe", True) or config.pipeline.review_fix_loop
        )
