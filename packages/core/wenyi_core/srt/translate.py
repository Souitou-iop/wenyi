...
def _flush_usage(
    ...
    *,
    ...
    if not increment["totals"]["calls"]:
        store._usage_checkpoint = current
...
def translate_srt(
    ...
    *,
    ...
    """Translate subtitles and flush completed model usage even after a safe pause."""
    state: dict[str, Any] = {}
    try:
        return _translate_srt_impl(
            source_path,
            config,
            client=client,
            out=out,
        ...
        if state:
            store = state["store"]
            _flush_usage(store, state["client"], store._usage_checkpoint, scope="srt_interrupted")
            store.update_manifest(status="interrupted")
        raise


def _translate_srt_impl(
    source_path: str,
    config: Config,
    *,
    client: LLMClient | None = None,
    ...
    with RunTimer("srt.translate") as timer:
        ...
        store = (
            SrtRunStore(storage.run_dir, storage=storage)
            if storage is not None
            else SrtRunStore.for_source(config.state_dir, source_path, target_language)
        )
        ...
        store._usage_checkpoint = usage_checkpoint
        if _state is not None:
            _state.update(store=store, client=llm)
        ...
        saved_translations = dict(final_translations)
        pending = [
            job
            for job in jobs
        ...
        # Human edits and committed cue results take priority over old raw batch caches.
        final_translations.update(saved_translations)
        total = len(pending)
        done = 0
        if progress:
