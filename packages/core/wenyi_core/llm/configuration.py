...
class TierRef(str):
    ...
    def __new__(cls, val: str, model: str | None = None, options: dict[str, Any] | None = None):
...
class LLMConfig(BaseModel):
    ...
    def validate_graph(self) -> LLMConfig:
        ...
        for name, ref in list(self.tiers.items()):
            if ref in self.models:
                m = self.models[ref]
                self.tiers[name] = TierRef(str(ref), model=m.model, options=m.options)
    ...
    def timeout(self) -> float | None:
        p = self.providers.get("default")
        return p.timeout if p else None

    @property
    def max_retries(self) -> int | None:
        p = self.providers.get("default")
        return p.max_retries if p else None

    @property
    def reasoning_style(self) -> str | None:
        p = self.providers.get("default")
        if p and p.model_extra:
            return p.model_extra.get("reasoning_style")
        return None
