...
def inspect_book(path: str, cover_directory: str, book_id: str) -> dict:
    ...
    if ext == ".fb2":
        ...
        for author in title_info.iter():
            if _local(author.tag) == "author":
                name = " ".join(
                    _texts(author, "first-name")
                    + _texts(author, "middle-name")
                    + _texts(author, "last-name")
                ).strip()
                if name:
                    authors.append(name)
        ...
        cover_link = next(
            (
                _attribute(image, "href")
                for coverpage in title_info.iter()
                if _local(coverpage.tag) == "coverpage"
                for image in coverpage.iter()
        ...
        if cover_id:
            binary = next(
                (
                    el
                    for el in root.iter()
                    if _local(el.tag) == "binary" and el.attrib.get("id") == cover_id
                ),
                None,
            )
    ...
    with archive as zf:
        ...
        for item_id, item in manifest.items():
            ...
            if (
                "cover-image" in props.split()
                or item_id == cover_id
                or ("cover" in item_id.lower() and media.startswith("image/"))
            ):
                cover_item = item
                break
