...
def _public_book(book: dict[str, Any]) -> dict[str, Any]:
    ...
    result["title"] = (
        book.get("title") or stored.get("title") or Path(book.get("filename") or "").stem
    )
...
class WebSettings(BaseModel):
    provider: Literal["deepseek", "openai", "openrouter", "openai-compatible", "ollama", "vllm"] = (
        "deepseek"
    )
    ...
    strong: TierSettings = Field(
        default_factory=lambda: TierSettings(model="deepseek-v4-pro", thinking=True)
    )
    cheap: TierSettings = Field(
        default_factory=lambda: TierSettings(model="deepseek-v4-flash", thinking=True)
    )
...
class TaskManager:
    ...
    def workspace(self, task: dict[str, Any]) -> RunStore | None:
        ...
        state_dir = Path(task.get("state_dir") or self.store.state_dir / task.get("book_id", ""))
    ...
    def public(self, task: dict[str, Any]) -> dict[str, Any]:
        ...
        result["outputs"] = [Path(path).name for path in task.get("outputs") or []]
    ...
    async def start(
        self, book: dict[str, Any], output_format: str = "epub", task_id: str | None = None
    ) -> dict[str, Any]:
        async with self.lifecycle_lock:
            return await self._start(book, output_format, task_id)

    async def _start(
        self, book: dict[str, Any], output_format: str, task_id: str | None
    ) -> dict[str, Any]:
        ...
        config_path = Path(
            previous.get("config_path")
            if previous and previous.get("config_path")
            else self.store.tasks_dir / f"{task_id}.yaml"
        )
        state_dir = Path(
            previous.get("state_dir")
            if previous and previous.get("state_dir")
        ...
        if previous and config_path.exists():
            ...
            if snapshot_provider != settings.provider or snapshot_base_url != current_base_url:
                raise HTTPException(
                    409,
    ...
    async def _run(
        self, task: dict[str, Any], book: dict[str, Any], output: Path, output_format: str
    ) -> None:
        settings = self.store.settings()
        ...
        try:
            ...
            while line := await process.stdout.readline():
                ...
                if event_type == "phase":
                    ...
                    task.update(
                        status="paused" if event.get("code") == "cancelled" else "failed",
            ...
            if process.returncode and task["status"] == "running":
                task.update(
                    status="failed", error=message[-2000:] or f"worker exited {process.returncode}"
                )
                self.save(task)
    ...
    async def shutdown(self) -> None:
        await asyncio.gather(
            *(self.stop(task_id) for task_id in tuple(self.jobs)), return_exceptions=True
        )
...
def _public_event(row: dict[str, Any]) -> dict[str, Any]:
    ...
    result.update(
        {key: value for key, value in row.items() if key in allowed and key not in {"ts", "event"}}
    )
    return result
...
def _public_usage(usage: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "by_tier": {name: normalize(slot) for name, slot in (usage.get("by_tier") or {}).items()},
        "by_stage": {name: normalize(slot) for name, slot in (usage.get("by_stage") or {}).items()},
    }


def create_app(data_dir: Path | None = None, web_dir: Path | None = None) -> FastAPI:
    store = WebStore(
        data_dir or Path(os.environ.get("WENYI_WEB_DATA", Path.home() / ".wenyi-webui"))
    )
    ...
    async def reject_cross_site_writes(request: Request, call_next):
        if request.method in {"POST", "PUT", "PATCH", "DELETE"}:
            ...
            if request.headers.get("sec-fetch-site") == "cross-site" or (
                origin is not None and hostname not in {"localhost", "127.0.0.1", "::1"}
            ):
                return JSONResponse(
                    status_code=403,
    ...
    def test_connection(request: TestConnectionRequest):
        ...
        saved = store.settings()
        api_key = request.api_key or (saved.api_key if request.provider == saved.provider else "")
    ...
    async def delete_book(book_id: str):
        async with manager.lifecycle_lock:
            ...
            if not book:
                raise HTTPException(404, "图书不存在")
            task_ids = sorted(
                task["id"] for task in store.tasks() if task.get("book_id") == book_id
            )
            with ExitStack() as locks:
                ...
                if any(
                    any(export.get("status") == "pending" for export in task.get("exports") or [])
                    for task in tasks
                ):
                    raise HTTPException(409, "重新导出完成前不能删除图书")
                for task in tasks:
                    config_path = Path(
                        task.get("config_path") or store.tasks_dir / f"{task['id']}.yaml"
                    )
    ...
    def get_groups():
        ...
        return [{**group, "book_count": counts.get(group["id"], 0)} for group in store.groups()]
    ...
    def rename_group(group_id: str, request: GroupRequest):
        ...
        if not group:
            raise HTTPException(404, "分组不存在")
        if any(
            item["id"] != group_id and item["name"].casefold() == name.casefold() for item in groups
        ):
            raise HTTPException(409, "分组名称已存在")
        group["name"] = name
        store.save_groups(groups)
        return {
            **group,
            "book_count": sum(book.get("group_id") == group_id for book in store.books()),
        }
    ...
    def update_task_analysis(task_id: str, request: AnalysisUpdateRequest):
        task, workspace = _workspace_or_409(manager, task_id)
        if task.get("status") == "running" and task.get("phase_code") in {"preparing", "prescan"}:
    ...
    def resolve_glossary_conflict(
        ...
        try:
            conflict = next(
                (item for item in glossary.open_conflicts() if item["id"] == conflict_id),
                None,
            )
            if conflict is None:
    ...
    def get_task_usage(task_id: str):
        ...
        return {
            ...
            "cache_available": (
                int(totals.get("cache_hit_tokens") or 0) + int(totals.get("cache_miss_tokens") or 0)
            )
            > 0,
        }
    ...
    async def create_task_export(task_id: str, request: ExportRequest):
        ...
        with manager.task_lock(task_id):
            ...
            output = (
                export_dir
                / f"{Path(book['filename']).stem}.zh{'-bi' if request.bilingual else ''}.{suffix}"
            )
            record = {
                "id": export_id,
        ...
        try:
            ...
            def fail(current):
                saved = next(
                    item for item in current.get("exports") or [] if item.get("id") == export_id
                )
        ...
        completed_record = next(
            item for item in completed_task.get("exports") or [] if item.get("id") == export_id
        )
        workspace.log_event(
    ...
    def download_task_export(task_id: str, export_id: str):
        task = manager.get(task_id)
        record = next(
            (item for item in task_exports(task) if item.get("id") == export_id),
            None,
        )
    ...
    def download_output(task_id: str, name: str):
        task = manager.get(task_id)
        output = next(
            (Path(path) for path in task.get("outputs", []) if Path(path).name == name), None
        )
